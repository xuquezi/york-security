...
public class RoleController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "roleName", value = "角色名模糊查询", dataType = "String",paramType = "query"),
    ...
    public PageResult getRolePageResult(@RequestParam(name = "search",defaultValue = "")String roleName, @RequestParam(name = "limit",defaultValue = "10") Integer pageSize, @RequestParam(name = "page",defaultValue = "1")Integer pageNum){
        // username，pageSize，pageNum都设置了默认值，不会为空
        log.info("当前页为："+ pageNum);
        log.info("每页显示记录数："+ pageSize);
        log.info("搜索名为："+ roleName);
        PageInfo pageInfo = roleService.findRolePageInfo(roleName, pageSize, pageNum);
